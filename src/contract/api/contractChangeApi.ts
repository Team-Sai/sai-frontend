...
function isChangeRequestDetail(value: unknown): value is ChangeRequestDetail {
  ...
  return (
    typeof d.changeRequestId === "number" &&
    (typeof d.newContractId === "number" || d.newContractId === null) &&
    typeof d.requesterName === "string" &&
    typeof d.rejectorName === "string" &&
    typeof d.requestedAt === "string" &&
    typeof d.status === "string" &&
    typeof d.currentMaturityDate === "string" &&
    typeof d.currentInterestRate === "number" &&
    typeof d.currentRepaymentType === "string" &&
    (typeof d.currentTerms === "string" || d.currentTerms === null) &&
    typeof d.currentMonthlyPayment === "number" &&
    typeof d.newMaturityDate === "string" &&
    typeof d.newInterestRate === "number" &&
    typeof d.newRepaymentType === "string" &&
    (typeof d.newTerms === "string" || d.newTerms === null) &&
    typeof d.newMonthlyPayment === "number" &&
    typeof d.changeReason === "string" &&
    typeof d.extendedMonths === "number" &&
    (typeof d.returnReason === "string" || d.returnReason === null)
  );
}
...
export async function getChangeRequestDetail(
  ...
  if (!response.ok) {
    throw new Error(
      await readErrorMessage(response, "변경 요청 정보를 불러오지 못했습니다."),
    );
  }
  ...
  if (!isChangeRequestDetail(data)) {
    throw new Error("변경 요청 정보 응답 형식이 올바르지 않습니다.");
  }
...
export async function rejectChangeRequest(
  ...
  if (!response.ok) {
    throw new Error(await readErrorMessage(response, "반려 처리에 실패했습니다."));
  }
}
...
export async function submitChangeApproval(
  ...
  const response = await authFetch(`/api/contracts/${contractId}/change-approval`, {
    method: "PATCH",
    body: form,
  });
  ...
  if (!response.ok) {
    throw new Error(
      await readErrorMessage(response, `서명 제출에 실패했습니다. (HTTP ${response.status})`),
    );
  }
}