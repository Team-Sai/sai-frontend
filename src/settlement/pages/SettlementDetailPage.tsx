...
import type { PaymentObligation, SettlementAccount, SettlementDetail, SettlementPaymentStatus } from '../types/settlement';
...
const dt=(v?:string|null)=>{if(!v)return '-';const value=new Date(v);if(Number.isNaN(value.getTime()))return '-';const dateLabel=`${value.getFullYear()}-${String(value.getMonth()+1).padStart(2,'0')}-${String(value.getDate()).padStart(2,'0')}`;const timeLabel=new Intl.DateTimeFormat('ko-KR',{hour:'2-digit',minute:'2-digit'}).format(value);return `${dateLabel} ${timeLabel}`};
const paymentDateTime=(v?:string|null)=>{if(!v)return null;const date=new Date(v);return{date:`${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}-${String(date.getDate()).padStart(2,'0')}`,time:new Intl.DateTimeFormat('ko-KR',{hour:'2-digit',minute:'2-digit'}).format(date)}};
const statusText=(v?:string)=>({IN_PROGRESS:'정산 중',CLOSED:'정산 완료'} as Record<string,string>)[v||'']||'-';
...
export default function SettlementDetailPage(){
  ...
  const [loadedSettlementId,setLoadedSettlementId]=useState<number|null>(null);
  const [busy,setBusy]=useState(false);
  const isBusy=busy||loadedSettlementId!==id;
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      setDetail(d);setStatus(s);setAccount(a);
  ...
  useEffect(()=>{
    ...
    fetchSettlementDetail(id).then(([d,s,a])=>{
      ...
      if(!cancelled)setLoadedSettlementId(id);
    });
  ...
  if(!Number.isFinite(id)) return <main className="page-shell settlement-detail-page"><div className="empty-state">정산 ID를 확인할 수 없습니다.</div></main>;
  if(!detail&&isBusy) return <main className="page-shell settlement-detail-page"><LoadingSkeleton className="loading-skeleton--page" rows={8} /></main>;

  async function sync(){
    try{setBusy(true);const r=await settlementApi.syncAll();setSyncTime(formatTransactionSyncTime(recordTransactionSync()));showToast(`동기화 완료: 자동반영 ${r.appliedCount??0}건, 확인필요 ${r.needsCheckCount??0}건, 미매칭 ${r.unmatchedCount??0}건`);await load();setReviewOpen(true)}catch(e){showToast(e instanceof Error?e.message:'거래내역 동기화에 실패했습니다.',true)}finally{setBusy(false)}
  }
  ...
  return <>
    <main className="page-shell settlement-detail-page">
      <section className="detail-heading"><div><div className="heading-badges"><span className={`badge ${detail?.settlementStatus==='CLOSED'?'badge-completed':'badge-progress'}`}>{statusText(detail?.settlementStatus)}</span><span className="badge badge-role">{roleText(detail?.role)}</span><span className="badge">{recurring?'정기정산':'공동정산'}</span></div><h1>{detail?.title||'정산 상세'}</h1></div><div className="heading-actions"><Link className="button button-secondary" to="/settlements">목록으로</Link>{detail?.role!=='MEMBER'&&<button className="button button-secondary settlement-sync-button" type="button" onClick={()=>void sync()} disabled={isBusy}>{busy&&<span className="button-spinner" aria-hidden="true" />}↻ 거래내역 동기화</button>}{detail?.role==='OWNER'&&<button className="button button-primary" type="button" disabled={detail.settlementStatus==='CLOSED'||!status.closable||isBusy} onClick={()=>setCloseModal(true)}>정산 마감</button>}</div></section>
      ...
      <div className="detail-grid"><aside className="side-column"><section className="panel"><div className="panel-header"><h2>정산 요청 정보</h2></div><div className="account-card"><div className="account-icon">₩</div><div><span>{account?.bankName||'수취 계좌'}</span><strong>{account?.maskedAccountNumber||'계좌 정보 없음'}</strong><small>{account?.accountHolderName||'-'}</small></div></div><dl className="info-list"><div><dt>분배 방식</dt><dd>{splitText(detail?.splitType)}</dd></div><div><dt>생성일</dt><dd>{dt(detail?.createdAt)}</dd></div><div><dt>최근 동기화</dt><dd>{syncTime}</dd></div><div><dt>확인 필요</dt><dd className="text-danger">{attentionCount}명</dd></div></dl></section>
        ...
        <section className="panel participant-panel"><div className="panel-header participant-panel-header"><div><h2>참여자별 입금 현황</h2><p>정산 생성 시 산정된 분담 금액과 실제 입금 반영 결과입니다.</p></div></div><div className="table-scroll"><table className="payment-table"><thead><tr><th>참여자</th><th>분담 금액</th><th>입금 금액</th><th>잔여 금액</th><th>최근 입금일</th><th>입금 상태</th></tr></thead><tbody>{obligations.map(o=>{const paidAt=paymentDateTime(o.latestPaymentAt);return <tr key={o.paymentObligationId || o.participantId}><td className="name-cell">{o.participantName||`참여자 #${o.participantId}`}</td><td>{money(o.expectedAmount)}원</td><td>{money(o.paidAmount)}원</td><td>{money(o.remainingAmount)}원</td><td>{paidAt?<span className="payment-date-time"><span>{paidAt.date}</span><span>{paidAt.time}</span></span>:'-'}</td><td><span className={`status-chip ${paymentClass(o)}`}>● {paymentText(o)}</span></td></tr>})}</tbody></table></div>{obligations.length===0&&<div className="empty-state">등록된 납부 대상이 없습니다.</div>}</section>
      </div>
    </main>
