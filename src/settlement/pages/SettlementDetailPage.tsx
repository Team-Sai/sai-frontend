...
const statusText=(v?:string)=>({IN_PROGRESS:'진행중',CLOSED:'완료'} as Record<string,string>)[v||'']||'-';
...
export default function SettlementDetailPage(){
  ...
  async function sync(){
    try{setBusy(true);const r=await settlementApi.syncAll();showToast(`동기화 완료: 자동반영 ${r.appliedCount??0}건, 확인필요 ${r.needsCheckCount??0}건, 미매칭 ${r.unmatchedCount??0}건`);await load();setReviewOpen(true)}catch(e){showToast(e instanceof Error?e.message:'거래내역 동기화에 실패했습니다.',true)}finally{setBusy(false)}
  }
  ...
  return <>
    <main className="page-shell settlement-detail-page">
      <section className="detail-heading"><div><div className="heading-badges"><span className={`badge ${detail?.settlementStatus==='CLOSED'?'badge-completed':'badge-progress'}`}>{statusText(detail?.settlementStatus)}</span><span className="badge badge-role">{roleText(detail?.role)}</span><span className="badge">{recurring?'정기정산':'공동정산'}</span></div><h1>{detail?.title||'정산 상세'}</h1></div><div className="heading-actions"><Link className="button button-secondary" to="/settlements">목록으로</Link>{detail?.role!=='MEMBER'&&<button className="button button-secondary" type="button" onClick={()=>void sync()} disabled={isBusy}>↻ 거래내역 동기화</button>}{detail?.role==='OWNER'&&<button className="button button-primary" type="button" disabled={detail.settlementStatus==='CLOSED'||!status.closable||isBusy} onClick={()=>setCloseModal(true)}>정산 마감</button>}</div></section>
